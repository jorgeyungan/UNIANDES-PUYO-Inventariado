...
public class crudMantenimiento {
    ...
    public static boolean save(clsMantenimiento mantenimiento){
        ...
        String sql = "INSERT INTO public.mantenimiento(fecha_mantenimiento,detalle)"
                + " VALUES(?,?)";
        ArrayList<Parametro> lstPar = new ArrayList<>();
        lstPar.add(new Parametro(1, mantenimiento.getFechamantenimiento()));
        lstPar.add(new Parametro(2, mantenimiento.getDetalle()));
        try {
    ...
    public static boolean update(clsMantenimiento mantenimiento) {
        ...
        String sql = "UPDATE public.mantenimiento " +
                "SET fecha_mantenimiento=?,detalle=? " + 
                "WHERE idmantenimiento=?";
        ArrayList<Parametro> lstPar = new ArrayList<>(); 
        lstPar.add(new Parametro(1, mantenimiento.getFechamantenimiento()));
        lstPar.add(new Parametro(2, mantenimiento.getDetalle()));
        lstPar.add(new Parametro(3, mantenimiento.getIdmantenimiento())); 
        try{
            res= AccesoDatos.ejecutaComando(sql, lstPar);
        }catch (Exception e) {
            System.out.println(e.getMessage());
    ...
    public static ArrayList<clsMantenimiento> findbyAll() {
        ...
        String sql = "SELECT idmantenimiento,fecha_mantenimiento,detalle "+
        ...
        try {
            ...
            while (cres.next())
            {
                ...
                mante.setFechamantenimiento(cres.getString("fecha_mantenimiento"));
                mante.setDetalle(cres.getString("detalle"));
                listado.add((mante));
            }
        } catch (Exception e) {
            System.out.println(e.getMessage());
